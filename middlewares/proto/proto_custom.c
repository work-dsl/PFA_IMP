...
/**
 * @brief 自定义协议帧格式描述
 * @note  协议格式：FA(1) + LEN(2) + PRODUCT(1) + CMD(1) + BOARD(1) + DATA(...) + CRC16(2) + 0x0D(1)
 */
 ...
 * @note  帧格式：FA + LEN(2) + PRODUCT + CMD + BOARD + DATA + CRC(2) + 0x0D
 *        固定开销为9字节（不含DATA部分）
 */
                     ...
                     const uint8_t *data, uint16_t data_len)
{
    ...
    return custom_build_frame_internal(out, out_size, product, cmd, board,
 ...
 * @retval >=0 构建的帧长度（字节数）
 * @retval -1 失败（缓冲区太小或参数无效）
 * @details 应答帧格式：FA + LEN(2) + PRODUCT + CMD + BOARD + ACK + DATA + CRC(2) + 0x0D
 *          固定开销为10字节（不含DATA部分）
